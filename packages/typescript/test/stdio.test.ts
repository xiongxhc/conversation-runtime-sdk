...
test("spawns without a shell", async () => {
  await withGateway(
    "emit({ type: 'ready', protocol_version: 3, status }); process.stdin.once('end', () => process.exit(0));",
...
test("continuously drains bounded stderr until the child exits", async () => {
  await withGateway(
    "emit({ type: 'ready', protocol_version: 3, status }); (async () => { const chunk = 'x'.repeat(65536); for (let index = 0; index < 8; index += 1) { if (!process.stderr.write(chunk)) await new Promise((resolve) => process.stderr.once('drain', resolve)); } process.exit(0); })();",
...
test("rejects client work when the gateway exits", async () => {
  await withGateway(
    "emit({ type: 'ready', protocol_version: 3, status }); process.stdin.once('data', () => setTimeout(() => process.exit(1), 10));",
    async ({ gatewayPath, configPath }) => {
      const transport = await StdioGatewayTransport.start({ gatewayPath, configPath });
...
test("discards buffered ready and responses when the process fails", async () => {
  await withGateway(
    "emit({ type: 'ready', protocol_version: 3, status }); setTimeout(() => { writeFileSync(`${process.argv[3]}.exit`, ''); process.exit(1); }, 10);",
    async ({ gatewayPath, configPath }) => {
      const transport = await StdioGatewayTransport.start({ gatewayPath, configPath });
...
test("discards buffered status responses when exit follows", async () => {
  await withGateway(
    "emit({ type: 'ready', protocol_version: 3, status }); process.stdin.once('data', () => { emit({ type: 'command_accepted', protocol_version: 3, request_id: 'request-1' }); emit({ type: 'status', protocol_version: 3, request_id: 'request-1', status }); writeFileSync(`${process.argv[3]}.exit`, ''); process.exit(1); });",
    async ({ gatewayPath, configPath }) => {
      const transport = await StdioGatewayTransport.start({ gatewayPath, configPath });
      const iterator = transport.messages[Symbol.asyncIterator]();
      await iterator.next();
...
test("closes an EOF-ignoring child with bounded termination and reaping", { timeout: 2_000 }, async () => {
  await withGateway(
    "process.stdin.resume(); process.on('SIGTERM', () => {}); setTimeout(() => process.exit(0), 700); emit({ type: 'ready', protocol_version: 3, status });",
    async ({ gatewayPath, configPath }) => {
      const transport = await StdioGatewayTransport.start({ gatewayPath, configPath });
      await transport.messages[Symbol.asyncIterator]().next();
      await Promise.race([
...
test("uses content-free errors for spawn and child stream failures", async () => {
  ...
  await withGateway(
    "emit({ type: 'ready', protocol_version: 3, status }); process.stdin.once('data', () => { process.stderr.write('private-stderr-value'); process.exit(1); });",
    async ({ gatewayPath, configPath }) => {
      const transport = await StdioGatewayTransport.start({ gatewayPath, configPath });
      const client = await RuntimeClient.connect(transport);
      await assert.rejects(
...
function framedReady(): Buffer {
  const payload = Buffer.from(JSON.stringify({ type: "ready", protocol_version: 3, status: {
    transport: "stdio",
