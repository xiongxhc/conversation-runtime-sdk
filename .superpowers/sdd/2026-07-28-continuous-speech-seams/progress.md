...
Task 4: added deterministic cross-stage regressions for external interruption, synthesis failure during active output, and output failure during active synthesis. Each test checks cleanup at terminal acceptance, exactly-one stage-correct terminal publication, and no later output invocation in its active-synthesis scenario; interruption also proves runtime reuse. The approved Task 3 worker already satisfied all three boundaries, and targeted premature-return mutations failed or hung, so `speech_worker.rs` remains unchanged. Five cancellation repetitions passed at 23/23 each; 80 runtime tests, strict runtime Clippy, format, and diff checks pass. Report: `task-4-report.md`.

Task 4 review round 1/5: added a deterministic three-segment regression that observes segment 1 valid-audio synthesis complete while output 0 is blocked and segment 2 remains blocked by the occupied capacity-one prepared queue. Output 0 failure cleans before exactly one `AudioOutput` terminal, discards queued segment 1 without invocation, prevents segment 2 synthesis, and permits runtime reuse. A mutation that ignored output failure and continued receiving queued audio failed with `queued segment 1 played after output failure`. Focused green, five cancellation repetitions at 24/24, 81 runtime tests, strict Clippy, format, and diff checks pass; `speech_worker.rs` remains unchanged.

Task 5 deterministic/documentation portion: the integrated loopback probe now preserves the exact formatted Chinese standard output while one raw HTTP speech request contains the normalized speech-only text. It proves one captured WAV/player launch, empty playback temporary storage, exactly-once ordered milestones, and final `status=completed`. The stale blocked-write fixture was staged so active playback is causally established before the oversized output failure. Focused probe CLI passed 12/12; the locked no-fail-fast workspace suite passed 239/239; formatting, strict workspace Clippy, diff, incomplete-marker, concrete private-path, and residual-process checks passed. Report: `task-5-report.md`.

Task 5 live check status: not yet performed. Apple Silicon listening evidence remains with the orchestrator; no subjective continuity, first-audible, or hardware-latency claim was made.

Task 5 final verification note: one no-fail-fast matrix run observed an unrelated Ollama first-delta timeout test reach the adapter-error path. The test passed immediately in isolation, and the subsequent complete workspace rerun passed 239/239. No unowned file was changed.
