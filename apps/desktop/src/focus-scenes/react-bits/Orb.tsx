...
export default function Orb(props: FocusSceneProps) {
    ...
    try {
      ...
      const program = new Program(gl, {
        ...
        uniforms: {
          ...
          hue: { value: stateHue(props.state) },
          hover: { value: 0 },
          rot: { value: 0 },
          hoverIntensity: { value: 0.08 + intensity * 0.18 },
      ...
      const stopAnimation = startVisibilityAwareAnimation({
        ...
        renderFrame: (time) => {
          ...
          lastTime = time;
          const targetPresence = statePresence(props.state) * intensity;
          presence += (targetPresence - presence) * 0.1;
          if (props.state === "speaking") rotation += delta * 0.3;
          program.uniforms.iTime.value = time * 0.001;
          program.uniforms.hover.value = presence;
  ...
  return (
    <div
      ...
      data-focus-scene="orb"
      data-voice-presence="integrated"
...
function stateHue(state: FocusSceneProps["state"]): number {
  if (state === "listening") return -45;
  if (state === "speaking") return 30;
  if (state === "interrupted") return 5;
  if (state === "error") return -110;
  return 0;
}

function statePresence(state: FocusSceneProps["state"]): number {
  if (state === "listening") return 0.45;
  if (state === "thinking") return 0.25;
